    ...
    <a href={href} target="_blank" rel="noopener noreferrer" className={`block p-4 rounded-xl border transition-all duration-300 group ${getStyle()}`}>
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-white group-hover:text-cyan-400 transition-colors">{label}</h3>
...
export default function MultiversePortal() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      if (res.ok) { const d = await res.json(); setAdminState({ isShopOpen: !!d.isShopOpen, isMaintenanceMode: !!d.isMaintenanceMode }) }
    }
  ...
  const handleSubmit = async () => {
    ...
    } catch (e) { alert("Transmission failed.") } finally { setIsTransmitting(false) }
  }

  // --- MAINTENANCE VIEW ---
  if (adminState.isMaintenanceMode) {
    return (
      <div className="min-h-screen bg-[#050810] flex flex-col items-center justify-center p-6 font-mono text-center">
        <div className="mb-8 p-4 border border-yellow-500/30 bg-yellow-500/5 rounded-xl">
          <div className="flex items-center justify-center gap-2 text-yellow-500 mb-2">
            <AlertTriangle className="animate-pulse" /> <span className="font-bold">SYSTEMS_OFFLINE</span>
          </div>
          <h1 className="text-4xl font-black italic text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">MULTIVERSE MOUSE</h1>
        </div>
        ...
        <p className="mt-8 text-slate-600 text-xs tracking-widest uppercase">&gt; Initializing Repair Sequences...</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#050810] grid grid-cols-1 lg:grid-cols-[300px_1fr_300px] gap-6 p-6">
      ...
      <main className="flex flex-col items-center py-4">
        ...
        <div className="w-full max-w-lg bg-slate-900/30 p-5 rounded-xl border border-slate-800/50 backdrop-blur-sm">
          ...
          {visibleName && <Input value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Alias..." className="bg-slate-950 border-slate-800 mb-3" />}
          <Textarea value={echoMessage} onChange={(e) => setEchoMessage(e.target.value)} placeholder="Submit Request..." className="bg-slate-950 border-slate-800 mb-4 resize-none min-h-[100px]" />
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2 text-xs text-slate-500"><Switch checked={visibleName} onCheckedChange={setVisibleName} /> VISIBLE_NAME</div>
          ...
          {submitSuccess && <p className="mt-4 text-cyan-400 text-center font-mono text-xs">✓ TRANSMISSION_RECEIVED</p>}
        </div>
      </main>
