...
def convert_flux_transformer_checkpoint_to_diffusers(checkpoint, **kwargs):
    ...
    # Some checkpoints use .weight instead of .scale for qk norm layers
    for k in list(checkpoint.keys()):
        if ".norm.query_norm.weight" in k or ".norm.key_norm.weight" in k:
            checkpoint[k.replace(".weight", ".scale")] = checkpoint.pop(k)
    ...
    for i in range(num_layers):
        ...
        # qk_norm (optional — some checkpoints omit these keys)
        _norm_q = checkpoint.pop(f"double_blocks.{i}.img_attn.norm.query_norm.scale", None)
        if _norm_q is not None:
            converted_state_dict[f"{block_prefix}attn.norm_q.weight"] = _norm_q
        _norm_k = checkpoint.pop(f"double_blocks.{i}.img_attn.norm.key_norm.scale", None)
        if _norm_k is not None:
            converted_state_dict[f"{block_prefix}attn.norm_k.weight"] = _norm_k
        _norm_added_q = checkpoint.pop(f"double_blocks.{i}.txt_attn.norm.query_norm.scale", None)
        if _norm_added_q is not None:
            converted_state_dict[f"{block_prefix}attn.norm_added_q.weight"] = _norm_added_q
        _norm_added_k = checkpoint.pop(f"double_blocks.{i}.txt_attn.norm.key_norm.scale", None)
        if _norm_added_k is not None:
            converted_state_dict[f"{block_prefix}attn.norm_added_k.weight"] = _norm_added_k
        # ff img_mlp
    ...
    for i in range(num_single_layers):
        ...
        # qk norm (optional — some checkpoints omit these keys)
        _sq_norm_q = checkpoint.pop(f"single_blocks.{i}.norm.query_norm.scale", None)
        if _sq_norm_q is not None:
            converted_state_dict[f"{block_prefix}attn.norm_q.weight"] = _sq_norm_q
        _sq_norm_k = checkpoint.pop(f"single_blocks.{i}.norm.key_norm.scale", None)
        if _sq_norm_k is not None:
            converted_state_dict[f"{block_prefix}attn.norm_k.weight"] = _sq_norm_k
        # output projections.
