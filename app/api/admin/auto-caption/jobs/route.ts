...
async function runJobBackground(jobId: string) {
  ...
  for (let i = job.nextIndex; i < ordered.length; i++) {
    const record = ordered[i]

    // Periodically check for cancellation
    if (i % 10 === 0 && !firstInRun) {
    ...
    if (!job.overwrite) {
      ...
      if (alreadyCaptioned || alreadyTagged) {
        skippedCount++
        newResults.push({ id: record.id, type: 'skip', imageUrl: record.imageUrl })
    ...
    if (!firstInRun) await new Promise(r => setTimeout(r, 800))
    firstInRun = false
...
export async function POST(req: Request) {
  ...
  const job = await prisma.autoFillJob.create({
    data: { mode, modelKey: modelKey ?? 'flash', overwrite, advanced, curatorContext, triggerWord, imageIds: ids, totalCount: ids.length },
  })

  after(async () => {
    await runJobBackground(job.id)
  })
